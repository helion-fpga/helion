...
## Release / stranger clone

Stranger path: `cargo test --workspace` then `helion-ide --headless examples/counter.sv` → `WNS_PS=9640`. Mac bundle: `packaging/macos` + `scripts/build-macos-app.sh`. `Info.plist` version must match `Cargo.toml`. Release artifacts gated at 100 MiB (`packaging/check-artifact-size.sh`); skip `examples/ip_ingest`; `helion-ide` in the `.app` is a wrapper over `Helion`. Stages / SOFT ≠ PASS: `docs/stages.md`.
