...
Ordered labs in `examples/README.md`. Empty-XDC `examples/counter.sv` gold is `WNS_PS=9640` (do not move). Headless: `./scripts/example-spine.sh`. Soft CPU (`examples/hcore`) is last. Helion-MM/ST only — no AXI-as-product.
