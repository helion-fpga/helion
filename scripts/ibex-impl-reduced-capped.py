#!/usr/bin/env python3
"""FM-HEL-TOP: capped reduced-Ibex implement. Never uncapped. Cap <=120s."""
...
rtl = os.path.join(root, "examples/ibex_reduced.sv")
out_bits = os.path.join(root, "target/ibex-reduced-impl.hbits")
...
print(f"IBEX_REDUCED_IMPL: cap={cap}s cmd={' '.join(cmd)}", flush=True)
...
try:
    while proc.poll() is None:
        if time.time() - t0 >= cap:
            try:
                ...
                proc.kill()
            print(f"\nIBEX_REDUCED_IMPL_CAP: killed after {cap}s", flush=True)
            try:
                proc.wait(timeout=2)
            except Exception:
                pass
            size = os.path.getsize(out_bits) if os.path.isfile(out_bits) else 0
            print(f"IBEX_REDUCED_IMPL: TIMEOUT exit=124 elapsed={cap}s hbits_bytes={size}", flush=True)
            sys.exit(124)
        time.sleep(0.2)
    t.join(timeout=1)
    ...
    size = os.path.getsize(out_bits) if os.path.isfile(out_bits) else 0
    print(f"IBEX_REDUCED_IMPL: exit={rc} elapsed={elapsed:.2f}s hbits_bytes={size} path={out_bits}", flush=True)
    sys.exit(rc)
