// Helion-ST registered count-trailing-zeros — 32b data path (not AXI).
// Loads 32b word via four Helion-ST bytes (st_valid + byte_sel), then on
// enable registers ctz = number of trailing zeros in din[31:0]. Real FF+LUT fabric.
// Pair to h_clz (leading zeros).
module h_ctz (
    input  wire       clk,
    input  wire       resetn,
    ...
    input  wire       enable,   // compute & register CTZ
    ...
    // Combinational 32b count-trailing-zeros (priority mux tree → LUTs)
    // din[0] is LSB; count==32 when din==0.
    wire [5:0] ctz =
          din[0]  ? 6'd0  :
          din[1]  ? 6'd1  :
          din[2]  ? 6'd2  :
